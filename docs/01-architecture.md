...
**P1 现状**：`api` 目前以宿主进程形态运行（`ragdemo serve`，只绑
127.0.0.1），不是上面拓扑图里的容器——诊断接口的维护成本要接近零
（adr/0010 后果 4），P1 阶段现写一份 Dockerfile 只会先添一份没人用得上
的维护负担。`infra/docker-compose.yml` 因此暂时没有 `api` 这一项。
容器化留到 P4 对外产品接口真正要走生产部署的时候一起做。
