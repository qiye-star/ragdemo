...
- [x] **D · 质量指标底座** —— `6e7d1c1`。六项检查实现（第七项"应到/实到对账"
  按计划留给阶段 F，数据依赖决定的，不是漏做）。用真实 Dagster 管线手工造过一个
  真孤儿块实测过阻断语义：`orphan_block_check` 失败后 `block_embeddings` 在同一次
  run 里完全没被调度，日志里根本不出现这一步——不是靠猜测/文档描述认定它会阻断。
- [x] **E · 时点泄漏抽样重放** —— `0f4b5ff`。E1–E6 全过。实现中发现并修正一个会让
  整套机制形同虚设的设计缺陷：探测点若每次都从块当前 `known_at` 现算，`known_at`
  一旦被篡改探测点会跟着偏移，篡改反而永远逃过检测——改成探测点只在首次抽中时
...
- [x] **G · C 档二次解析路由** —— `1f1715a`。G2–G6 全过（真实 Dagster 物化 +
  单测验证）；G1（真实月度 C 档触发率 < 3%）需要生产数据跑满一个月才能验收，
  本阶段交付的是达成这条验收所需的机制本身（策略表 + 路由 + 月度预算 + 埋点），
  不是这个百分比数字本身。
  开发中自己写完单测后发现并改正一处真 bug：月度预算的花费汇总最初按
  `quality_metric.computed_at`（这一行被写入的物理时刻）过滤，应该按
  `partition_date`（业务发生时刻）——否则回填历史分区时，所有历史月份的
  C 档花费都会被记成"回填当天"这一个月，与 `core.document.ingested_at`
  不能进 `known_at` 过滤是同一类陷阱，改用 `partition_date` 后单测才真正
  覆盖到"跨月边界"这个场景。
  设计上的一处调整：`tier_c_reparse` 资产最初在内部直接 `TextInParser(...)`
  现场构造解析器，测试没有办法注入 `httpx.MockTransport`，只能测到"配置
  缺失时跳过"这一条分支，测不到真正的重解析路径——改成解析器由
  `definitions.py` 的 `tier_c_parser_resource` 注入（`TEXTIN_BASE_URL`
  未配置时注入 `None`），与其余资源同一个模式，G2/G3/G4/G5 才有办法在不
  连真实网络的前提下验证。
  G6（重试队列）需要一张独立于 `core.document` 的表：`ParseRetryable`/
